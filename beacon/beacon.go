...
func (beacon *Beacon) MarkandSweepContainersandVolumes() error {
	...
	if sweepCntErr != nil {
		errString = errString + fmt.Sprintf("sweep-err:  %s", sweepCntErr.Error())
	}

	if reportCntErr != nil {
		errString = errString + fmt.Sprintf("report-err: %s", reportCntErr.Error())
	}

	if sweepVolErr != nil {
		errString = errString + fmt.Sprintf("sweep-err:  %s", sweepVolErr.Error())
	}

	if reportVolErr != nil {
		errString = errString + fmt.Sprintf("report-err: %s", reportVolErr.Error())
	}
...
func (beacon *Beacon) runReport(command string) error {
	...
	go func() {
		...
		case tsa.ReportVolumes:
			...
			beacon.Logger.Debug("sucessfully-reported-volume-handles", lager.Data{"cmd-output": string(reportHandleBytes)})
		}
...
func (beacon *Beacon) runSweep(command string) error {
	...
	go func() {
		...
		case tsa.SweepVolumes:
			...
			beacon.Logger.Debug("sucessfully-sweeped-volume-handles")
		default:
